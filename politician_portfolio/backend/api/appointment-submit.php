...
require_once '../database/connection.php'; // Path to DB connection
...
try {
    ...
    // Validate mobile number (Nepali format: 10 digits starting with 96, 97, or 98)
    if (!preg_match('/^[9][6-8][0-9]{8}$/', $input['mobile_number'])) {
        throw new Exception('Invalid mobile number format (must be 10 digits starting with 96, 97, or 98)');
    }
